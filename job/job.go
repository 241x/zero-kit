...
// 处理器（Handler）必须实现幂等或显式容忍重复，并且尊重 ctx 的取消
// （作业超时、取消或执行器停止时 ctx 会被关闭），否则会造成 goroutine
// 泄漏且作业无法及时终止。
//
// 取消延迟：对 running 作业调用 Cancel 只是更新库中状态，处理器要等到
// 下一次心跳（默认 HeartbeatInterval 10s）发现状态变化后才会收到 ctx 取消，
// 因此取消生效存在最多约一个心跳间隔的延迟。
//
// 处理器分发：Executor 支持按作业类型注册处理器（Register），未注册的
// 类型回退到 NewExecutor 传入的默认处理器；两者都没有时会按重试策略
// 重新入队，超过 MaxAttempts 后标记失败。
//
